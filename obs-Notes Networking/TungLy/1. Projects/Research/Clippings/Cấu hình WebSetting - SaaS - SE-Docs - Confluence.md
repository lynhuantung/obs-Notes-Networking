---
title: Cấu hình WebSetting - SaaS - SE-Docs - Confluence
...
- Mục tiêu của triển khai SaaS (Phần mềm dưới dạng Dịch vụ) theo kiến trúc multi tenant trên Kubernetes (k8s) là cung cấp linh hoạt việc triển khai các ứng dụng SaaS. Kubernetes cung cấp các tính năng như tự phục hồi, tự động mở rộng và quản lý tài nguyên để giúp các nhà phát triển và quản trị viên có thể triển khai và quản lý các ứng dụng SaaS của họ một cách hiệu quả.

## 1.2 MỤC ĐÍCH TÀI LIỆU

- Giúp IT cấu hình webSetting.json trong SaaS

## 2\. HƯỚNG DẪN CHI TIẾT

## 2.1 HRM.Presentation.Main

`"` **Identity\_ConnectionString** `": "[c](https://login.microsoftonline.com;https//login.live.com)huổi kết nối đến identity server"`
...
`"` **IsMultitenant** `": cấu hình giúp nhận biết đây là multi tenant`

` "` **TenantCode** `": cấu hình này giúp biết tenant code cụ thể (không cần cấu hình cụ thể 1 tenant, hệ thống tự động thiết lập)`
...
```
{
    ...
    "AuthMicrosoftAccount": {
        "Enable": true,
        "DisplayName": "Đăng nhập bằng Microsoft" ,
