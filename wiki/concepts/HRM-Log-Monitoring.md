---
description: "Bộ công cụ giám sát và đọc log HRM: Event Viewer, IIS log, SQL log, theo thứ tự từ dễ đến chuyên sâu."
type: concept
tags:
  - hrm
  - monitoring
...
- "[[wiki/sources/2026-05-04-log-review-bitex]]"
---

# HRM — Log & Monitoring Toolkit

> Ghi chú nguyên tử: các công cụ chẩn đoán lỗi HRM theo thứ tự từ dễ đến sâu.
...
## Log Review — Gom nhóm log WindowsService & API SC

Dùng khi: cần xem log của nhiều service (WindowsService, API SC) trên cùng một giao diện.

**Cơ chế**: `Sys_LogReviewController.GetListRoot()` tự detect hoặc đọc config `LogFolders`.

**Cách B — Config tường minh (khuyến nghị production)**:
```xml
<add key="LogFolders"
     value="C:\Deploy\HRM.Presentation.WindowsService\Log|C:\Deploy\HRM.SC.Service.Api\Log" />
```

**Lưu ý quan trọng**: Mọi `FullPath` trả về client **phải encrypt** — nếu không, `ReadDirectory()`
sẽ decrypt ra path rác → forbidden. Xem chi tiết: [[wiki/sources/2026-05-04-log-review-bitex]]
