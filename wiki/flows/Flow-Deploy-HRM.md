---
description: "Quy trình deploy HRM: upbuild IIS (dọn source, giải nén, warmup Redis), Kubernetes (VnPay), smoke test và rollback khi lỗi."
type: flow
tags:
  - deploy
  - iis
...
Quy trình deploy HRM gồm 2 luồng chính: **Upbuild** (cập nhật source code lên server) và **Warmup** (đảm bảo hệ thống sẵn sàng phục vụ sau khi restart IIS pools). Áp dụng cho mọi môi trường (DEV, UAT, Production), thực hiện mỗi lần release hoặc hotfix.
...
```mermaid
flowchart TD
    A[Chuan bi Artifacts / Build va dong goi rar]
    B[Upload len AWS S3 / DB backup Source Config]
    C[STOP tat ca IIS Pool va Site]
    D[Backup RequestInformation / 4 thu muc bat buoc]
    E[Xoa source cu / PrecompiledApp.config va bin]
    F[Giai nen build moi / giu nguyen web.config]
    G[START lai Pool va Site]
    H[Kiem tra Warmup / redis-cli HGETALL WarmupStatus]
    W{6 services warmed up?}
    WAIT[Cho them max 120s]
    I[Smoke Test / Login Main va Portal check version]
    T{Test pass?}
    J[Confirm va Notify / Release Note thong bao team]
    RB[Rollback / restore backup]
...
```mermaid
flowchart TD
    K1[Build Docker image va push registry]
    K2[Update deployment manifest / tag version moi]
    K3[kubectl apply / Rolling Update]
    K4[Monitor pod health / kubectl get pods]
    K5{Pods Running?}
    K6[Kiem tra Traefik routing / subdomain dung service]
    K7[Warmup check / Redis WarmupStatus key]
    K8[Smoke Test va Notify]
    KFAIL[kubectl rollout undo / rollback deployment]
