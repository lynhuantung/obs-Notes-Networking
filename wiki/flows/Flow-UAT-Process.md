---
description: "Quy trình UAT HRM: chuẩn bị môi trường, đào tạo key user, test từng phân hệ, ghi lỗi Issue Log, fix và re-test đến khi sign-off và golive."
type: flow
tags:
  - uat
  - testing
...
UAT (User Acceptance Testing) là giai đoạn khách hàng **tự kiểm tra** toàn bộ hệ thống trước khi go-live. Đây là cửa ải cuối cùng — nếu UAT fail, không được go-live. Áp dụng trong Giai đoạn 4 của quy trình triển khai HRM. Case study: VnPay Phase 4 (06–07/2025).
...
```mermaid
flowchart TD
    A[Chuan bi moi truong UAT: Deploy DB mau tai khoan test]
    B[Dao tao Key User theo tung phan he truoc UAT 1-2 ngay]
    C[UAT vong 1: Test tung phan he theo thu tu dependency]
    D[Ghi loi vao Issue Log: ID muc do screenshot steps]
    E[VnR Fix + RE-TEST noi bo: staging QC pass build release]
    F[Deploy ban fix len UAT]
    G[Khach hang Re-test: verify tung loi da bao]
    H{Con loi moi hoac reopen?}
    I[UAT vong 2: Toan he thong Test end-to-end integration]
    J[Sign-off UAT: Bien ban nghiem thu PM KH ky]
    K{Dieu kien sign-off dat?}
    L[Nghiem thu va Go-live: Hypercare 2-4 tuan]
    REOPEN[Reopen loi: vong lai fix]
