---
description: "Kiến trúc xác thực HRM gồm JWT, SSO, OAuth2, LDAP và IdentityServer4, mô tả flow login đa phương thức."
type: architecture
tags:
  - authentication
  - jwt
...
## Mô hình 2b — Extension: 2FA (Two-Factor Authentication)

Xem chi tiết: [[wiki/architecture/2FA-IDS4-Architecture]] và [[wiki/flows/Flow-2FA-Login]]

**2FA chỉ áp dụng cho Mô hình 2b (IDS4):**
- Sau khi validate password → IDS4 check user có bật 2FA không
- Nếu có → yêu cầu nhập OTP (TOTP 6 chữ số, chu kỳ 30s) từ Google/Microsoft Authenticator
- Chỉ sau khi OTP hợp lệ → cấp JWT

**Điều kiện bắt buộc 2FA:**
...
**Security components thêm:**
- Secret Key lưu DB encrypted per-user
- Rate Limit chống brute force OTP
- Recovery Code (N mã, dùng 1 lần) khi mất thiết bị
- Audit log: login success/fail, OTP fail, disable 2FA
- NTP sync bắt buộc (Clock Drift ±30–60s)
...
- [[wiki/sources/SSO-HRM-Protocols]] — Giao thức SSO (SAML/OIDC/LDAP) + hướng dẫn Okta chi tiết
...
- [[wiki/architecture/2FA-IDS4-Architecture]] — Kiến trúc 2FA chi tiết (component, TOTP, Recovery Code)
- [[wiki/flows/Flow-2FA-Login]] — Các flow 2FA: setup, login, recovery, quản lý
- [[wiki/sources/2fa-ids4-solution]] — Tài liệu giải pháp 2FA đầy đủ
- [[wiki/sources/p7m3k-umc-sso-microsoft-bien-ban-hop]] — Biên bản họp UMC SSO Microsoft 14/05/2026
- [[wiki/flows/p7m3k-flow-sso-microsoft-umc]] — Luồng SSO Microsoft OAuth2/OIDC cho UMC
- [[wiki/projects/UMC-Project]] — Dự án UMC tích hợp SSO
