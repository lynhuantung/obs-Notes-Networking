...
VD:
```
aws s3 cp d:\readme.txt s3://vnr-pmns-build-version/ --endpoint-url https://s3-hni.sds.vnpaycloud.vn
```
...
```
aws s3 ls s3://vnr-pmns-build-version/ --endpoint-url https://s3-hni.sds.vnpaycloud.vn
```
...
- Upload file
```bash
aws s3 cp D:\tmp\VnPay-s3\test.rar s3://vnr-pmns-build-version/ --endpoint-url https://s3-hni.sds.vnpaycloud.vn
```
